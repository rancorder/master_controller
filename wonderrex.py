...
# -*- coding: utf-8 -*-
"""
wonderrex.py - WonderREX エンタープライズスクレイパー v2.0

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Architecture Decision Record (ADR)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

ADR-001: Shopify JSON API活用
=========================================
- Status: ACCEPTED
- Context: WonderREXはShopifyベースで、products.json APIを提供
- Decision: REST APIを直接呼び出し、HTML解析を回避
- Consequences:
  + 高速・安定なデータ取得
  + サイト構造変更への耐性

ADR-002: URL順序をshop_config.jsonと一致
=========================================
- Status: VERIFIED
- Context: shop_config.json確認結果
  url_index: 0 → 時計
  url_index: 1 → カメラ
- Decision: スクリプト内URLをこの順序に維持（既に正しい）
- Consequences: 整合性維持

【SLI/SLO定義】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- SLI: API応答成功率
- SLO: 99.9% (JSON APIのため高信頼性)
...
class Constants:
    ...
    TARGET_URLS: Final[Tuple[Tuple[str, str], ...]] = (
        ("時計", "https://wonderrex-online.com/collections/%E6%99%82%E8%A8%88/products.json"),      # url_index: 0
        ("カメラ", "https://wonderrex-online.com/collections/%E3%82%AB%E3%83%A1%E3%83%A9/products.json"),  # url_index: 1
    )
...
class ProductData:
    ...
    @classmethod
    def create(
        cls, name: str, price: int, url_index: int, category: str = "", rank: int = 0
    ) -> ProductData:
        product_hash = hashlib.md5(f"{name}_{price}".encode("utf-8")).hexdigest()[:8]
        return cls(
            name=name, price=price, url_index=url_index, category=category,
...
class StructuredLogger:
    def __init__(self, name: str = "wonderrex_scraper", level: int = logging.INFO):
...
class ProductValidator:
    @staticmethod
    def validate_price(price_raw: Any) -> Optional[int]:
        """Shopify API価格バリデーション"""
        try:
...
class WonderrexJsonParser:
    """Shopify JSON APIパーサー"""
    
    def __init__(self, validator: ProductValidator, logger: Optional[LoggerProtocol] = None):
        self._validator = validator
        self._logger = logger or StructuredLogger()
    
    def parse(self, data: Dict[str, Any], url_index: int, category: str) -> List[ProductData]:
        products: List[ProductData] = []
        api_products = data.get("products", [])
        
        self._logger.debug(f"URL index {url_index} ({category}): {len(api_products)}件のAPI商品")
        
        for rank, product in enumerate(api_products, start=1):
            parsed = self._parse_product(product, url_index, category, rank)
            if parsed:
    ...
    def _parse_product(
        self, product: Dict[str, Any], url_index: int, category: str, rank: int
        ...
        try:
            ...
            name = self._validator.validate_name(title)
            if not name:
                return None
            ...
            return ProductData.create(
                name=name, price=price, url_index=url_index, category=category, rank=rank
            )
...
class WonderrexScraper:
    """WonderREXスクレイパー（Shopify JSON API）"""
    ...
    def __init__(
        ...
        self._parser = WonderrexJsonParser(self._validator, self._logger)
    ...
    def scrape(self) -> ScrapeResult:
        ...
        self._logger.info(f"スクレイピング開始: {len(self._target_urls)} カテゴリ")
        ...
        try:
            ...
            for url_index, (category, url) in enumerate(self._target_urls):
                # URL Index出力
                print(f"---URL_INDEX:{url_index}---")
                
                try:
                    products = self._scrape_single_category(url, url_index, category)
                    ...
                    self._logger.error(f"URL index {url_index} ({category}) エラー: {e}")
                    continue
    ...
    def _scrape_single_category(self, url: str, url_index: int, category: str) -> List[ProductData]:
        def _scrape() -> List[ProductData]:
            with self._circuit_breaker.protect():
                data = self._http_client.get_json(url)
                return self._parser.parse(data, url_index, category)
        
        return self._retry_policy.execute_with_retry(_scrape, f"scrape_{category}")
...
def main() -> int:
    ...
    scraper = WonderrexScraper(logger=logger)
