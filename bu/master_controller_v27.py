...
# -*- coding: utf-8 -*-
"""
Master Controller v26 - P1個別スナップショット対応版
...
class StableDataExtractor:
    ...
    def _extract_product_from_line(
        ...
        # v27.1修正: IWC等の短いブランド名に対応
        if not product_name or len(product_name) == 0:
            return None
        if len(product_name) <= 2 and price < 100:
            return None
    ...
    def cleanup(self, retention_hours: int = 24) -> int:
        ...
        if deleted_count > 100:
            try:
                ...
                conn.execute("VACUUM")
                conn.close()
                self.logger.debug("🗑️ VACUUM実行完了")
            except Exception as e:
                # VACUUMの失敗は無視（次回に実行される）
                pass
...
class Tier2CycleController:
    ...
    def verify_snapshot_updates(self) -> List[str]:
        """スナップショット更新を検証（v26: P2共有ファイルから読み込み）"""
        current_time = datetime.now()
        not_updated = []
        
        try:
            # P2共有ファイルを読み込み
            p2_snapshot_path = self.memory_system.snapshot_dir / P2_SHARED_SNAPSHOT
            snapshot_data = self.memory_system._load_snapshot_file(p2_snapshot_path)
...
class FinalStableMasterController:
    """最終安定版マスターコントローラー v26（P1個別スナップショット対応）"""
    
    VERSION: Final[str] = "26"
    ...
    async def start_async(self) -> None:
        ...
        print("=" * 60)
        print("🔧 v26新機能: P1スクリプト用の個別JSONファイル")
...
def main() -> None:
    ...
    print("=" * 40)
    print("🔧 v26 - P1個別スナップショット対応版")
    print("=" * 40)
    print("")
    print("【主な改善点】")
    print("✅ P1スクリプト用の個別JSONファイル（競合解消）")
    print("✅ P2は共有ファイル維持（p2_shared.json）")
    print("✅ snapshotsディレクトリへの統合配置")
    print("✅ ファイルごとのロック機構")
    print("")
